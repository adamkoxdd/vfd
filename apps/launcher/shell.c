...
static const char *view_name(View view) {
    switch (view) {
        ...
        case VIEW_EVENTS: return "EVENTS";
...
static void load_root(App *a) {
    ...
    add_submenu(a, VIEW_EVENTS, "Events", "Recent desktop notifications");
...
static void parse_desktop_file(App *a, const char *path) {
    ...
    if (!clean[0]) return;
    if (terminal) snprintf(command, sizeof command, "vfdterm sh -lc \"%.*s\"", (int)sizeof(command) - 24, clean);
...
static void load_desktop(App *a) {
    ...
    add_action(a, "Open VFD Config", "Edit the VFD project", "vfdterm nvim ~/Projects/vfd");
    add_action(a, "Daemon Status", "Show vfdd status in a terminal", "vfdterm sh -lc 'vfdctl status; echo; read -r -p \"Press Enter...\"'");
}


static void load_events(App *a) {
    char response[8192], ignored[64];
    vfd_ipc_request("EVENTS READ", ignored, sizeof ignored);
    if (vfd_ipc_request("EVENTS", response, sizeof response) != 0) {
        add_action(a, "Daemon unavailable", "Cannot read event history", "vfdterm vfdctl status");
        return;
    }
    if (!strncmp(response, "EMPTY", 5)) {
        add_action(a, "No recent events", "Notification history is empty", "true");
        return;
    }
    char *save=NULL;
    for(char *line=strtok_r(response,"\n",&save);line;line=strtok_r(NULL,"\n",&save)){
        char *parts[5]={0}; int n=0; char *inner=NULL;
        for(char *v=strtok_r(line,"\t",&inner);v&&n<5;v=strtok_r(NULL,"\t",&inner))parts[n++]=v;
        if(n<4)continue;
        char label[MAX_LABEL], detail[MAX_LABEL];
        snprintf(label,sizeof label,"%s // %s",parts[2],parts[3]);
        snprintf(detail,sizeof detail,"%s%s%s",parts[1],(n>4&&parts[4][0])?" // ":"",(n>4)?parts[4]:"");
        add_action(a,label,detail,"true");
    }
    add_action(a,"Clear Event History","Remove all stored desktop events","vfdctl events clear");
}
...
static void load_items(App *a) {
    ...
    switch (a->view) {
        ...
        case VIEW_EVENTS: load_events(a); break;
...
static void render(App *a) {
    ...
    VfdStatusBar status = {left, help, "VFD SHELL 0.6", {0, a->height - 24, a->width, 24}};
...
int main(void) {
    ...
    app.ui = vfd_ui_open();
    if (!app.ui) { fprintf(stderr, "Shell: cannot open display\n"); return 1; }

    int mx = 0, my = 0, mw = 0, mh = 0;
    vfd_ui_monitor_geometry(app.ui, 0, &mx, &my, &mw, &mh);
    VfdWindowConfig config = {
        mx + (mw - app.width) / 2, my + (mh - app.height) / 3,
        app.width, app.height, "VFD Shell", "vfdshell", app.theme.font, false, 0
    };
    app.win = vfd_window_create(app.ui, &config);
    if (!app.win || load_colors(&app)) { fprintf(stderr, "Shell: window initialization failed\n"); return 1; }
